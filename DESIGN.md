...
| `ListRow` | 46px, 제목 15/500(Figma ListRow 11:2 기준, S6 정정) / 메타 11 dimmed, 우측 금액 모노 14/600(의미색), 행 구분 `--moeum-hair-2`, 아이콘 박스 없음(카테고리 색은 제목 앞 6px 점). 예외 = 관리 목록(카테고리·고정지출)의 `lead` — 18px 글리프(사용자가 고른 아이콘 · 카테고리 색 · stroke 2 · 장식이라 `aria-hidden`), 선택기 16개 밖 이름은 같은 자리 6px 점. 사용자 아이콘이 보이는 유일한 자리(배치4 pick A) |
