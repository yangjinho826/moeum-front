 ...
 * "{가계부} 멤버 N" 목록(이름 / 역할 · 이메일) → 소유자만 "멤버 추가"(가입한 이메일) · 소유자 외 행 끝 "내보내기".
 * 백엔드: 추가 = 수락 절차 없이 바로 멤버, 추가·내보내기 = 소유자만, 소유자 본인은 못 나감,
 * 내보내도 그 사람이 남긴 기록은 남음(soft delete).
 */
export default function MembersSection({ householdId, inSheet = false }: MembersSectionProps) {
  ...
  const handleInvite = async (e: FormEvent) => {
    ...
    if (!isValidEmail(target)) {
      showEmailError(tAuth("email_format_message"));
      return;
    }
    ...
    try {
      ...
      if (error instanceof ApiResponseError && error.errorCode === "CM004") {
        showEmailError(t("user_not_found"));
        return;
      }
      notifications.show({
              ...
              action={
                canRemove ? (
                  <UnstyledButton
                    ...
                    // 글자 13 · 히트 44(20 + 12·12) — 위아래 패딩을 음수 마진으로 상쇄해 행 높이는 그대로(DESIGN §4)
                    style={{
                      fontSize: 13,
                      lineHeight: "20px",
                      fontWeight: 600,
